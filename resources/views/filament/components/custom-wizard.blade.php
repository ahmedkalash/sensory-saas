    ...
    @if (! $isHeaderHidden)
        ...
        >
            @foreach ($steps as $step)
                @php
                    $stepId = $step->getId();
                    $qId = str_replace('q_', '', $stepId);
                    $isCompletedCondition = str_starts_with($stepId, 'q_')
                        ? "\$wire.get('data.draft_answers.{$qId}') != null"
                        : "getStepIndex(step) > {$loop->index}";
                @endphp
                <li
                    wire:key="wizard-step-{{ $stepId }}"
                    class="fi-sc-wizard-header-step"
                    x-bind:class="{
                        'fi-active': getStepIndex(step) === {{ $loop->index }},
                        'fi-completed': {!! $isCompletedCondition !!},
                    }"
                >
                    <button
                        type="button"
