    ...
    <style>
        ...
        * {
        ...
        .grid-table th, .grid-table td {
            border: 1px solid #e5e7eb; /* خط رفيع للجدول العادي */
            padding: 8px 6px; /* تم تصغير هذا الرقم لتقليل الفراغ */
            vertical-align: middle; /* لضبط النص في المنتصف عمودياً وعدم تركيزه في الأعلى */
            text-align: center; /* لتوسيط الأرقام والشارات أفقياً */
        }
        
        .thick-grid th, .thick-grid td {
            border: 3px solid #6b7280; /* خصيصاً لجدول الملخص */
        }
        .grid-table th {
            background: #eef2ff;
            color: #312e81;
        ...
        <table class="info-grid">
            ...
            <tr>
                ...
                <td class="info-cell">
                    ...
                    <span class="info-value">{{ $evaluation->evaluation_date?->format('Y-m-d') }} - {{ $evaluation->specialist_name }}</span>
                </td>
            </tr>
        </table>
    </div>
                ...
                @else
                    <div class="dimension-block">
                        ...
                        <div class="weakness-list">
                            <div class="weakness-text">توصيات لولي الامر:</div>
                            @foreach ($measurement['dimensions'] as $dim)
                                @foreach ($dim['weaknesses'] as $w)
                                        @foreach($w['recommendations'] as $recommendation)
                                            <div class="weakness-item" style="margin-bottom: 2px;">
                                               - {{ $recommendation }}
                                            </div>
                                        @endforeach
                                @endforeach
                            ...
                            <div class="weakness-text">أهداف:</div>
                            @foreach ($measurement['dimensions'] as $dim)
                                @foreach ($dim['weaknesses'] as $w)
                                    @foreach($w['goals'] as $goal)
                                        <div class="weakness-item" style="margin-bottom: 2px;">- {{ $goal }}</div>
                                    @endforeach
                                @endforeach
                            @endforeach


                            <div class="weakness-text">أنشطة:</div>
                            @foreach ($measurement['dimensions'] as $dim)
                                @foreach ($dim['weaknesses'] as $w)
                                        @foreach($w['activities'] as $activity)
                                            <div class="weakness-item" style="margin-bottom: 2px;">- {{ $activity }}</div>
                                        @endforeach
                                @endforeach
                            @endforeach
                        </div>
